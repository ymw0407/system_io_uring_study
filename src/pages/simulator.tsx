...
import * as s from './simulator.css';
...
export default function Simulator() {
  ...
  /* Run: compute all steps, then auto-play from the start */
  const handleRun = useCallback(() => {
    const trace = generateTrace(tracePreset, traceLength);
    const simResult = runSimulation(trace, params);
    setResult(simResult);
    setPosition(0);
    setPlaying(true);
  ...
  /* Auto-advance position while playing */
  useEffect(() => {
  ...
  /* Show [0, position]: chart grows from left to right */
  const visibleRange: [number, number] = [0, position];

  return (
    <div className={s.page}>
      <header className={s.header}>
        <h1 className={s.title}>Adaptive I/O Simulator</h1>
        <p className={s.subtitle}>
          피드백 없음 — 고정 트레이스에서 정책의 추적 능력을 검증한다
        </p>
      </header>
